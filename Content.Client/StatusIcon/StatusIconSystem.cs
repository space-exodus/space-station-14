...
public sealed class StatusIconSystem : SharedStatusIconSystem
{
    ...
    public bool IsVisible(Entity<MetaDataComponent> ent, StatusIconData data)
    {
        ...
        if ((data.ShowTo != null && !_entityWhitelist.IsValid(data.ShowTo, viewer)) || (data.ShowToNetEntities != null && (!viewer.HasValue || !data.ShowToNetEntities.Contains(EntityManager.GetNetEntity(viewer.Value)))))
            return false;
