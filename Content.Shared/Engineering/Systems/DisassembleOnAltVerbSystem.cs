...
using Robust.Shared.Prototypes; // Exodus-FoldedPosters
...
public sealed partial class DisassembleOnAltVerbSystem : EntitySystem
{
    ...
    private void OnDisassembleDoAfter(Entity<DisassembleOnAltVerbComponent> entity, ref DisassembleDoAfterEvent args)
    {
        ...
        var ev = new DisassembleEntityEvent(metadata.EntityPrototype?.ID, entity);
...
{
    public EntProtoId? DisassembledEntProto;

    public EntityUid CreatedEntity;

    public DisassembleEntityEvent(EntProtoId? disassembledEntProto, EntityUid createdEntity)
    {
